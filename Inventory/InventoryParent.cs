...
public class InventoryParent : MonoBehaviour
{
    ...
    public void AddInventoryItem(Inventoryitemidentity ob)
    {
        ...
        var childs = itemob.GetComponentsInChildren<Transform>();
        foreach (Transform child in childs)
        {
            if (!child.GetComponent<GrabPoint>())
            {
                child.gameObject.layer = item.layer;
                if (child.GetComponent<MeshRenderer>())
                child.GetComponent<MeshRenderer>().material = _InventoryButtonList[0].GetHighlightMaterial;
                child.transform.localPosition = new Vector3(child.transform.localPosition.x, 0.01f, child.transform.localPosition.z);
            }
        }
